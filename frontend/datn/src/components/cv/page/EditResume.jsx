...
import TitleInput from '../component/inputs/TitleInput';
...
import ProjectsDetailFrom from './Forms/ProjectDetailFrom';
...
const EditResume = () => {
  ...
  const validateAndNext = (e) => {
    ...
    switch (currentPage) {
      case "profile-info":
        ...
        if
          (!designation.trim()) errors.push("Designation is required");
      ...
      case "additionalInfo":
        ...
        if (
          resumeData.interests.length === 0 || !resumeData.interests[0]?.trim()
        ) {
  ...
  const fetchResumeDetailsById = async () => {
    console.log("fetchResumeDetailsById đang chạy");
    try {
      const response = await axiosInstance.get(
        API_PATHS.RESUME.GET_BY_ID(resumeId)
      );
      console.log("Resume API response:", response.data); // THÊM DÒNG NÀY
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      const updateResponse = await updateResumeDetails(thumbnailLink, profilePreviewUrl);

      // 3. Kiểm tra response
      if (uploadResponse.status === 200 && updateResponse?.status === 200) {
    ...
    <DashboardLayout >
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className=" bg-white rounded-lg border border-purple-100 overflow-hidden">
            ...
            <div className="mx-5">
              ...
              <div className="flex items-end justify-end gap-3 mt-3 mb-5">
                ...
                <button className="btn-small" onClick={validateAndNext}
                  disabled={isLoading}>
          ...
          <div ref={resumeRef} className='h-100[vh]'>
            {/* Resume Template */}
      ...
      >
        <div
          ref={resumeDownloadRef} className="w-[98vw] h-[90vh]">
          <RenderResume
            templateId={resumeData?.template?.theme || ''}
            resumeData={resumeData}
            colorPalette={resumeData?.template?.colorPalette || []}
